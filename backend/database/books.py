...
class BooksDB(DatabaseManager):
    ...
    def add_book(self, schema: BookSchema) -> None:
        result = self.db.table(table_name=self.__table).insert(schema.model_dump(mode='json', exclude_none=True)).execute()
        log.fire.info(f'book added successfully: {result}')
        return result

    def update_book(self, schema: BookSchema) -> None:
        result = self.db.table(table_name=self.__table).update(schema.model_dump(mode='json', exclude_none=True)).eq(column='id', value=schema.id).execute()
        log.fire.info(f'book: {schema.id} updated successfully')
        return result

    def delete_book(self, book_id: UUID4) -> None:
        result = self.db.table(table_name=self.__table).delete().eq(column='id', value=book_id).execute()
